...
def verificaClientes(listaClientes):
    primeiro_cliente = listaClientes[0]
    maiorAltura = primeiro_cliente
    menorAltura = primeiro_cliente
    maiorPeso = primeiro_cliente
    menorPeso = primeiro_cliente    

    for cliente in listaClientes:
        if cliente['Altura'] > maiorAltura['Altura']:
            maiorAltura = cliente
        
        if cliente['Altura'] < menorAltura['Altura']:
            menorAltura = cliente

        if cliente['Peso'] > maiorPeso['Peso']:
            maiorPeso = cliente

        if cliente['Peso'] < menorPeso['Peso']:
            menorPeso = cliente

    print(f'Maior Altura: {maiorAltura["Altura"]}')
    print(f'Menor Altura: {menorAltura["Altura"]}')
    print(f'Maior Peso: {maiorPeso["Peso"]}')
    print(f'Menor Peso: {menorPeso["Peso"]}')
